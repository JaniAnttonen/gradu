...
\begin{enumerate}
  ...
  \item VC.Com(\(m\)) \(\rightarrow\) \(\tau, com\) Given an input \(m\) = (m1 ... mn) output a commitment \(com\) and advice \(\tau\).

  \item VC.Open(\(com, m, i, \tau\)) \(\rightarrow\) \(\pi\) On input \(m \in M\) and \(i \in [1, n]\), the commitment \(com\), and advice \(\tau\) output an opening \(\pi\) that proves \(m\) is the \(i\)th committed element of \(com\).

  \item VC.Verify(\(com, m, i, \pi\)) \(\rightarrow\) \(0/1\) On input commitment \(com\), an index \(i \in [n]\), and an opening proof \(\pi\) output \(1\) (accept) or \(0\) (reject).

\end{enumerate}
%TODO: Add a formal definition of vector commitments and regular commitments (use an example like Pedersen commitments)
%TODO: Add new bibliography, refer to sources
