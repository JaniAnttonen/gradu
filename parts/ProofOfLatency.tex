...
The use case that drove me to proceed working on Proof of Latency is an idea that there could be a trustless way of telling to other peers how much latency you have to another peer. Today, if one peer were to tell you that it had a 10ms latency to another peer, there would be no quarantees of the 10ms latency holding true. Now, if we could make a proof of that latency using cryptography, you could tell by almost certain that the told latency is true, and nobody is trying to advertise you peers that have a malicious agenda.
...
% TODO Cite?
There have been proposals for a VDF-as-a-function system, in which less performant peers could query for a VDF calculation if they don't have the means to do so in a time frame small enough themselves. A benchmark query using PoL could also be a part of such a system. 
...
Also, I believe the work is not cryptographically as secure as it could be, and the field is progressing at a mindnumbing pace. Converting the used cryptographical primitives from RSA to ECC would be the logical step in trying to achieve quantum resistance. The idea behind this thesis is to think of a new creative way of using verifiable delay functions by defining a protocol, not necessarily using the most robust cryptography available. 
